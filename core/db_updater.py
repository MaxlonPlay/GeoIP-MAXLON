...
class DBUpdater:
    ...
    def _download_file(self):
        print(f"🌍 Scaricando il nuovo CSV da: {self.csv_url}")
        try:
    ...
    def _validate_csv(self):
        ...
        try:
            with open(self.temp_filename, 'r', encoding='utf-8') as f:
                header = f.readline()
                if not all(col in header for col in ['network', 'asn', 'organization', 'country']):
                    print("❌ Errore: Intestazione CSV non valida o colonne mancanti.")
                    return False
                first_data_line = f.readline()
                if not first_data_line.strip():
                    print("⚠️  Avviso: Il file CSV sembra vuoto dopo l'intestazione.")
            print("✅ Validazione CSV di base superata.")
            return True
        except Exception as e:
            print(f"❌ Errore durante la validazione CSV: {e}")
            return False
    ...
    def _update_files(self):
        ...
        try:
            if os.path.exists(self.data_filename):
                ...
                if old_md5 == new_md5:
                    print("ℹ️  Il nuovo CSV è identico a quello esistente. Nessun aggiornamento necessario.")
                    os.remove(self.temp_filename)
                    return True
            ...
            print(f"❌ Errore durante l'aggiornamento dei file: {e}")
            return False

    def _clear_cache(self):
        print("🗑️  Cancellazione file di cache...")
        try:
            cache_files = list(self.cache_dir.glob(f"{Path(self.data_filename).stem}_daemon_v*.cache"))
            if not cache_files:
            ...
            print(f"❌ Errore durante la cancellazione della cache: {e}")

    def update_database(self):
        print("\n=== Avvio Aggiornamento Database CSV ===")
        db_directory = Path(self.data_filename).parent
        if not db_directory.exists():
            print(f"📂 Creazione directory: {db_directory}")
            db_directory.mkdir(parents=True, exist_ok=True)
        if not self._download_file():
            print("🛑 Aggiornamento fallito: Impossibile scaricare il file CSV.")
            return False
        if not self._validate_csv():
            print("🛑 Aggiornamento fallito: Validazione CSV fallita.")
            os.remove(self.temp_filename)
            return False
        if not self._update_files():
            print("🛑 Aggiornamento fallito: Errore durante la sostituzione dei file.")
            return False
