...
def main() -> None:
    if len(sys.argv) < 2:
        ...
        print("   curl http://<il-tuo-ip>:" + str(os.getenv('DAEMON_WEB_PORT', 8888)) + "/8.8.8.8")
    ...
    if command == '--server':
        ...
        try:
            ...
            print("[ERROR] File 'networks.csv' non trovato. Tentativo di aggiornamento del database...")
            updater = DBUpdater()
            if updater.update_database():
                print("[INFO] Aggiornamento database completato. Riprovo ad avviare il daemon.")
                try:
                    lookup_service = GeoLookupService()
                except FileNotFoundError:
                    print("[ERROR] Impossibile caricare il database anche dopo l'aggiornamento. Uscita.")
                    sys.exit(1)
            else:
                print("[ERROR] Aggiornamento database fallito. Impossibile avviare il daemon.")
                sys.exit(1)
        ...
        try:
            ...
            print("[ERROR] Il processo del daemon non è stato trovato. Potrebbe essere già terminato.")
        ...
        updater = DBUpdater()
        if updater.update_database():
            print("[INFO] Aggiornamento database completato. Riavvia il daemon per caricare i nuovi dati.")
        else:
        ...
        try:
            ...
            updater = DBUpdater()
            if updater.update_database():
                print("[INFO] Aggiornamento database completato. Riprovo a caricare il database.")
                try:
                    lookup_service = GeoLookupService()
                except FileNotFoundError:
                    print("[ERROR] Impossibile caricare il database anche dopo l'aggiornamento. Uscita.")
                ...
                print("[ERROR] Aggiornamento database fallito. Impossibile procedere in modalità standalone.")
                sys.exit(1)
